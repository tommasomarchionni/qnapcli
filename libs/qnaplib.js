 ...
 * @param strictSSL
 */
let makeQnapBeep = (protocol, host, port, sid, timeout, strictSSL) => {
    return rp({
        ...
        return xml2js(res).then(result => {
            if (result['QDocRoot']['authPassed'][0] == 1) {
                utils.printInfo('Beep');
            } else {
                utils.printError('You are not authorized!');
            }
        });
    });
};
 ...
 */
let getSid = (protocol, host, port, username, password, timeout, strictSSL) => {
    let encode_password = ezEncode(utf16to8(password));
    return rp({
        uri:`${protocol}://${host}:${port}/cgi-bin/authLogin.cgi?user=${username}&pwd=${encode_password}`,
        timeout: timeout,
        strictSSL: strictSSL == 1
    }).then(res => {
        return xml2js(res).then(result => {
